...
public class ContentStorage : MXFInterchangeObject
{
	...
	protected internal override void read(Map tags)
	{
		...
		while (it.hasNext())
		{
			...
			{
			case 6401:
				packageRefs = MXFMetadata.readULBatch(_bb);
				break;
			case 6402:
				essenceContainerData = MXFMetadata.readULBatch(_bb);
				break;
			default:
				Logger.warn(String.format(new StringBuilder().append("Unknown tag [ ContentStorage: ").append(ul).append("]: %04x")
	...
	[LineNumberTable(48)]
	public virtual UL[] getPackageRefs()
	{
		return packageRefs;
	}

	[LineNumberTable(52)]
	public virtual UL[] getEssenceContainerData()
	{
